...
public class WolfEditorTest {
    @InjectMocks
    private WolfEditor editor;
    @Mock
    private UI ui;
    ...
    public void should_create_inventory() {
        ...
        when(ui.readString("Input wolf nickname")).thenReturn(nickname);
        when(ui.readInt("Input food count")).thenReturn(food);
        when(ui.readYesOrNo("Is wolf alfa")).thenReturn(true);
